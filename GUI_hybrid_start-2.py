...
Battery_ready = DigitalOutputDevice(18)
...
def buttons():
    ...
    if error_current.value == True:
        error_overcurrent()
    if error_voltage.value == True:
        error_overvoltage()
...
def error_overcurrent():
    print("error_overcurrent on")
...
def error_overvoltage():
    print("error_overvoltage on")
...
def battery_charge(battery_charge_from_MC):
    if battery_charge_from_MC == True:
       battery_charge.value = "Battery:Ready"
    elif battery_charge_from_MC == False:
        battery_charge.value = ("Battery:Charging")
    else:
        battery_charge.value = ("Battery:Calculating")
...
error_exit_BtN = PushButton(errorWindow, command= main_menu , text="Exit" , align = "left", width = "fill", height="fill",visible = False)
    #Date/Time
...
dateTime_info.repeat(1000,date_clock)#set to 60000 to change every minute
    ...
    #Power Settings
battery_charge = Text(app,text = "Battery:Calculating",align = "top",width = "fill",visible = True, size =30)
